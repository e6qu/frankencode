...
test("provider loaded from env variable", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect((providers["anthropic"].options.headers as Record<string, string>)["anthropic-beta"]).toBeDefined()
    },
  })
})
...
test("enabled_providers restricts to only listed providers", async () => {
  ...
  await Instance.provide({
    ...
    init: async () => {
      Env.set("ANTHROPIC_API_KEY", "test-api-key", Instance.directory)
      Env.set("OPENAI_API_KEY", "test-openai-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
      expect(providers["anthropic"]).toBeDefined()
...
test("env variable takes precedence, config merges options", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("ANTHROPIC_API_KEY", "env-api-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
      expect(providers["anthropic"]).toBeDefined()
...
test("multiple providers can be configured simultaneously", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("ANTHROPIC_API_KEY", "test-anthropic-key", Instance.directory)
      Env.set("OPENAI_API_KEY", "test-openai-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("provider with multiple env var options only includes apiKey when single env", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("MULTI_ENV_KEY_1", "test-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("provider with single env var includes apiKey automatically", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("SINGLE_ENV_KEY", "my-api-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("disabled_providers and enabled_providers interaction", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("ANTHROPIC_API_KEY", "test-anthropic", Instance.directory)
      Env.set("OPENAI_API_KEY", "test-openai", Instance.directory)
      Env.set("GOOGLE_GENERATIVE_AI_API_KEY", "test-google", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("provider env fallback - second env var used if first missing", async () => {
  ...
  await Instance.provide({
    ...
    init: async () => {
      ...
      Env.set("FALLBACK_KEY", "fallback-api-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("provider options are deeply merged", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(providers["anthropic"].options.timeout).toBe(30000)
      expect((providers["anthropic"].options.headers as Record<string, string>)["X-Custom"]).toBe("custom-value")
...
test("custom model inherits npm package from models.dev provider config", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("OPENAI_API_KEY", "test-api-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("custom model inherits api.url from models.dev provider", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("OPENROUTER_API_KEY", "test-api-key", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("model variants can be customized via config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect((model.variants!["high"].thinking as Record<string, number>).budgetTokens).toBe(20000)
    },
  })
})
...
test("Google Vertex: retains baseURL for custom proxy", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("GOOGLE_APPLICATION_CREDENTIALS", "test-creds", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
...
test("cloudflare-ai-gateway loads with env variables", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      Env.set("CLOUDFLARE_ACCOUNT_ID", "test-account", Instance.directory)
      Env.set("CLOUDFLARE_GATEWAY_ID", "test-gateway", Instance.directory)
      Env.set("CLOUDFLARE_API_TOKEN", "test-token", Instance.directory)
    },
    fn: async () => {
      const providers = await Provider.list()
