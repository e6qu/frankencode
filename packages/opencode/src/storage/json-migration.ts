...
export namespace JsonMigration {
  ...
  export async function run(sqlite: Database, options?: Options) {
    ...
    function insert(
      values: Record<string, string | number | boolean | null | object>[],
    ...
    const projectValues = [] as Record<string, string | number | boolean | null | object>[]
    ...
    const sessionValues = [] as Record<string, string | number | boolean | null | object>[]
    ...
    for (let i = 0; i < todoFiles.length; i += batchSize) {
      ...
      const values = [] as Record<string, string | number | boolean | null | object>[]
      for (let j = 0; j < batch.length; j++) {
        const data = batch[j]
        if (!data) continue
    ...
    const permValues = [] as Record<string, string | number | boolean | null | object>[]
    ...
    const shareValues = [] as Record<string, string | number | boolean | null | object>[]
