...
export namespace EditGraph {
  ...
  export function commit(input: {
    ...
    const nodeID = Identifier.ascending("part") // reuse part prefix for compact IDs
    const head = getHead(input.sessionID)
  ...
  export async function checkout(
    ...
    const headPath = buildPathToRoot(head.node_id)
    // Build path from target to root
    const targetPath = buildPathToRoot(targetNodeID)
  ...
  function buildPathToRoot(nodeID: string): Node[] {
    const path: Node[] = []
